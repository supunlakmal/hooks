...
export type UseAsyncAbortableActions<Args extends unknown[] = unknown[]> = UseAsyncActions<Args> & {
	/**
...
export function useAsyncAbortable<T, E = Error, Args extends unknown[] = unknown[]>( // Add export
    ...
    const memoizedFn = useCallback(async (...args: Args): Promise<T> => {
    ...
    const execute = useCallback(async (...params: Args): Promise<void> => {
        setState(() => ({
