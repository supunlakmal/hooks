...
interface UseIdleDetectionOptions {
  ...
  onChange?: (states: { userState: UserIdleState; screenState: ScreenIdleState }) => void;
...
export function useIdleDetection(options: UseIdleDetectionOptions = {}): UseIdleDetectionReturn {
  ...
  const [permissionState, setPermissionState] = useState<IdlePermissionState | null>(null);
  ...
  const requestPermission = useCallback(async (): Promise<IdlePermissionState> => {
    if (!isSupported) {
    ...
    try {
      ...
      const currentError = err instanceof Error ? err : new Error('Failed to request idle permission');
  ...
  const startDetector = useCallback(async () => {
    ...
    if (permissionState !== 'granted') {
        ...
        if (status !== 'granted') {
            ...
            optionsRef.current.onError?.(new Error('Idle detection permission not granted.'));
            return;
        }
    }
    ...
    try {
      ...
      detector.onchange = () => {
        ...
        optionsRef.current.onChange?.({ userState: detector.userState, screenState: detector.screenState });
      };

      await detector.start({ threshold: optionsRef.current.threshold, signal: abortControllerRef.current.signal });
  ...
  return { userState, screenState, permissionState, isActive, error, requestPermission, startDetector, isSupported };
}
